...
public class LevelOne  extends Level implements Screen {    
    ...
    public LevelOne(CS400Game game) {
        ...
        messages.add("A warning light."); //"These warning lights are getting on my nerves."
        messages.add("A warning light.");
            ...
            for (int i = 0; i < objects.size; i++) {
                if (objects.get(i).contains(Gdx.input.getX(), Gdx.input.getY())) {
                    if (textureIndex == 0 && i < 3) {// wall 0
                        ...
                        /*if (!inPuzzle && !inNote)
                            CS400Game.manager.get("sfx/message.wav", Sound.class).play();*/
                    }

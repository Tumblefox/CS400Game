...
public class LevelOne implements Screen {    
    ...
    private Array<TextureRegion> walls;
    private TextureRegion currentWall;
    ...
    boolean hasPass, hasKey, hasStrips, hasNote, hasFoil, hasPowder, hasRust, hasThermite;
    boolean disarmed; 
    ChemistryPuzzle puzzle = new ChemistryPuzzle();
    ...
    public void clickHandler() {
        if (Gdx.input.justTouched()) {
           ...
           if (textureIndex == 1) {
               ...
               if (objects.get(4).contains(Gdx.input.getX(), Gdx.input.getY()) && hasFoil && !hasPowder) {
                   inventory.add("Aluminum Powder");
                   inventory.removeValue("Foil", false);
